...
export const UpdateIdentifier: EmailTemplateComponent = ({
  user,
  otp,
  identifier,
  newIdentifier,
  ...
  if (otp) {
    const link = `${clientUrl}/auth/verify?identifier=${identifier}&newIdentifier=${newIdentifier}&purpose=${otp.purpose}&secret=${otp.secret}&type=${otp.type}`;
    return (
      <Layout previewText={`${subjectType} change request`}>
        ...
        {newIdentifier && (
          <>
            <Text className="text-base text-gray-900">
              Current {subjectType}: <strong>{identifier}</strong>
            </Text>
            <Text className="text-base text-gray-900">
  ...
  return (
    <Layout previewText={`${subjectType} updated`}>
      ...
      {newIdentifier && (
        <>
          <Text className="text-base text-gray-900">
            Previous {subjectType}: <strong>{identifier}</strong>
          </Text>
          <Text className="text-base text-gray-900">
            New {subjectType}: <strong>{newIdentifier}</strong>
          </Text>
